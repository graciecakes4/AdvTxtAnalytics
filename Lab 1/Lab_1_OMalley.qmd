...
ggplot(mpg, aes(displ, hwy)) + # create a scatter plot
  geom_point() +
  labs(title = "Displacement vs. Highway MPG Scatterplot", x = "Displacement (litres)", 
...
ggplot(mpg, aes(displ, hwy)) + # build a linear regression model scatter plot
  geom_point() +
  labs(title = "Displacement vs. Highway MPG Regression Model", x = "Engine Size", 
...
housing <- read.table("http://www.jaredlander.com/data/housing.csv", sep = ",", 
                      header = TRUE, stringsAsFactors = FALSE) # read in outside data
names(housing) <- c("Neighborhood", "Class", "Units", "YearsBuilt", "SqFt", "Income",
                    "IncomePer-SqFt", "Expense", "ExpensePerSqFt", "NetIncome", "Value",
                    "ValuePerSqFt", "Boro") # rename columns
head(housing)
```
