...
# download texts and set up corpus
ia_book_texts <- gutenbergr::gutenberg_download(ia_gutenberg_ids$gutenberg_id)
...
```{r building corpus}
book_texts_combined <- book_texts %>% 
    dplyr::group_by(gutenberg_id) %>% 
    dplyr::summarise(text = paste0(text, collapse = " "))
book_texts_combined
# re-downloaded books that are having issues
downloaded_books <- unique(book_texts_combined$gutenberg_id)
missing_books <- dplyr::setdiff(valid_books$gutenberg_id, downloaded_books)
downloaded_books_list = list()
for (i in downloaded_books) {
    if (i %in% combined_ids$gutenberg_id) {
        downloaded_books_list[[as.character(i)]] <- combined_ids %>% 
            dplyr::filter(gutenberg_id == i) %>% 
            dplyr::select(gutenberg_id, title)
    }
}
downloaded_books_df <- bind_rows(downloaded_books_list)
options(gutenbergr.mirror = "http://www.gutenberg.lib.md.us")
book_text_retry <- gutenbergr::gutenberg_download(valid_books$gutenberg_id)

book_corpus <- quanteda::corpus(book_texts, text_field = "text")
summary(book_corpus)
```

Now I am attempting to download directly from the gutenberg website. I was able to download a sample of 10 texts from Iowa's banned books list for the subgroup comparison.
