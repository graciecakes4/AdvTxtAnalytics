...
```{r florida tfidf}
# create a corpus and clean text a final time
fl_corpus <- tm::Corpus(tm::VectorSource(fl_book_texts_df$text))
fl_corpus <- tm::tm_map(fl_corpus, tm::removePunctuation)
fl_corpus <- tm::tm_map(fl_corpus, tm::removeWords, enhanced_stopwords)

# create a document term matrix
fl_dtm <- tm::DocumentTermMatrix(fl_corpus)

# create a frequency matrix dataframe
fl_dtm_matrix <- as.matrix(fl_dtm)
fl_term_freq <- colSums(fl_dtm_matrix)
fl_term_freq_df <- data.frame(term = names(fl_term_freq), freq = fl_term_freq)
...
```{r iowa tfidf}
# create a corpus and clean text a final time
ia_corpus <- tm::Corpus(tm::VectorSource(ia_book_texts_df$text))
ia_corpus <- tm::tm_map(ia_corpus, tm::removePunctuation)
ia_corpus <- tm::tm_map(ia_corpus, tm::removeWords, enhanced_stopwords)

# create a document term matrix
ia_dtm <- tm::DocumentTermMatrix(ia_corpus)

# create a frequency matrix dataframe
ia_dtm_matrix <- as.matrix(ia_dtm)
ia_term_freq <- colSums(ia_dtm_matrix)
ia_term_freq_df <- data.frame(term = names(ia_term_freq), freq = ia_term_freq)
...
### Florida Data Visualization {.tabset}

```{r florida tf viz}
# Select top 20 most frequent words
fl_top_term_freq <- fl_term_freq_df %>%
  dplyr::arrange(desc(freq)) %>% 
  dplyr::top_n(20, wt = freq)

# Plot
ggplot2::ggplot(fl_top_term_freq, ggplot2::aes(x = stats::reorder(term, freq), y = freq)) +
...
This is where I left off - need to figure out what is wrong with TF-IDF Scores why is it giving me numbers instead of words?
...
fl_dtm_tidy <- tidytext::tidy(fl_dtm)  # returns columns: document, term, count

# compute TF-IDF
fl_tfidf <- fl_dtm_tidy %>%
  dplyr::group_by(term) %>%
  dplyr::summarise(freq = sum(count)) %>%
  tidytext::bind_tf_idf(term, document = term, n = freq) 
...
# plot TF-IDF
ggplot2::ggplot(fl_top_tfidf, ggplot2::aes(x = stats::reorder(term, tf_idf), y = tf_idf)) +
